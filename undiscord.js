...
/* --- Color safety layer (popup only) ---
   Some Discord builds/themes no longer define older vars (ex: --text-normal),
   which makes Undiscord inherit black text. These fallbacks force readable colors. */
#undiscord {
  --ud-text: var(--text-normal, var(--text-default, var(--text-primary, #dbdee1)));
  --ud-muted: var(--text-muted, var(--header-secondary, #b5bac1));
  --ud-link: var(--text-link, var(--text-link-normal, #00a8fc));

  --ud-surface-high: var(--background-surface-high, var(--background-secondary, #1e1f22));
  --ud-tertiary: var(--background-tertiary, var(--background-secondary-alt, #2b2d31));

  --ud-input-bg: var(--input-background, var(--background-secondary, #1e1f22));
  --ud-input-border: var(--input-border, var(--background-modifier-accent, rgba(78,80,88,.3)));

  --ud-border: var(--border-subtle, var(--background-modifier-accent, rgba(78,80,88,.3)));

  /* Make default text readable even if Discord tokens are missing */
...
#undiscord.container,
#undiscord .container {
  background-color: var(--ud-surface-high);
  border-radius: 8px;
...
/* Header */
#undiscord .header {
  background-color: var(--ud-tertiary);
  height: 48px;
  align-items: center;
  min-height: 48px;
  padding: 0 16px;
...
#undiscord .header .icon {
  color: var(--interactive-normal, var(--ud-muted));
  margin-right: 8px;
  flex-shrink: 0;
  width: 24;
  height: 24;
}
#undiscord .header .icon:hover { color: var(--interactive-hover, var(--ud-text)); }
#undiscord .header h3 {
  font-size: 16px;
  line-height: 20px;
  font-weight: 500;
  font-family: var(--font-display);
  color: var(--header-primary, var(--ud-text));
  flex-shrink: 0;
  margin-right: 16px;
}
...
#undiscord .header .vert-divider {
  width: 1px;
  height: 24px;
  background-color: var(--ud-border);
  margin-right: 16px;
  flex-shrink: 0;
}
...
/* Ensure sidebar group headers are readable (General / Filter / etc.) */
#undiscord summary {
  color: var(--interactive-normal, var(--ud-text));
}
...
/* Links */
#undiscord a { color: var(--ud-link); text-decoration: none; }
...
#undiscord button {
  ...
  background-color: var(--button-secondary-background, var(--background-secondary-alt, #2b2d31));
}
...
/* lookFilled colorPrimary */
#undiscord .accent { background-color: var(--brand-experiment, #5865f2); }
#undiscord .danger { background-color: var(--button-danger-background, #da373c); }
#undiscord .positive { background-color: var(--button-positive-background, #23a559); }
#undiscord .info { font-size: 12px; line-height: 16px; padding: 8px 10px; color: var(--ud-muted); }

/* Make the log text readable (this is the big one in your screenshot) */
#undiscord #logArea { color: var(--ud-text); }
...
#undiscord .scroll::-webkit-scrollbar-thumb {
  ...
  border-radius: 4px;
  background-color: var(--scrollbar-thin-thumb, rgba(78,80,88,.5));
  min-height: 40px;
}
#undiscord .scroll::-webkit-scrollbar-track {
  border-color: var(--scrollbar-thin-track, transparent);
  background-color: var(--scrollbar-thin-track, transparent);
  border: 2px solid var(--scrollbar-thin-track, transparent);
}
/* fade scrollbar */
#undiscord .scroll::-webkit-scrollbar-thumb,
...
#undiscord.redact x:not(:active) {
  ...
  background-color: var(--primary-700, rgba(88,101,242,.35)) !important;
  cursor: default;
  user-select: none;
}
...
#undiscord .log-verb { color: var(--ud-muted); }
...
`;
    ...
    function initUI() {
        ...
        function mountBtn() {
            ...
            // 1) Find the Channel header (this is stable)
            const header =
                  document.querySelector('section[aria-label="Channel header"]') ||
                  document.querySelector('header[aria-label="Channel header"]') ||
                  document.querySelector('[aria-label="Channel header"]');

            if (!header) return false;

            // 2) Find the toolbar container (Discord uses hashed classnames, but "toolbar" is consistent)
            let toolbar =
                header.querySelector('div[class*="toolbar"]') ||
                header.querySelector('[role="toolbar"]');

            // Fallback: find a known button and climb to a reasonable parent
            if (!toolbar) {
                const anchor =
                      header.querySelector('[aria-label="Threads"]') ||
                      header.querySelector('[aria-label="Pinned Messages"]') ||
                      header.querySelector('[aria-label="Show Member List"]') ||
                      header.querySelector('[aria-label="Notification Settings"]');

                toolbar = anchor ? anchor.parentElement : null;
            }

            if (!toolbar) return false;

            // 3) De-dupe
            const existing = toolbar.querySelector('#undicord-btn');
            if (existing && existing !== btn) existing.remove();

            // Ensure the button isn't collapsed by layout
            btn.style.display = 'flex';
            btn.style.alignItems = 'center';
            btn.style.flex = '0 0 auto';

            // 4) Insert before the search chunk if it's a direct child of toolbar
            const directSearchChild = [...toolbar.children].find((child) => {
                // Search block typically contains a contenteditable combobox
                if (child.querySelector('[aria-label="Search"]')) return true;
                if (child.querySelector('[contenteditable="true"][role="combobox"]')) return true;
                // Or has "search" in its hashed classname
                if ([...child.classList].some((c) => c.toLowerCase().includes('search'))) return true;
                return false;
            });

            if (directSearchChild) toolbar.insertBefore(btn, directSearchChild);
